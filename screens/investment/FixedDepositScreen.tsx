...
export default function FixedDepositScreen() {
  ...
  const investment =
    ...
    investmentOptions.find((inv) => inv.type === "fixed_deposit");
  ...
  const calculateReturns = () => {
    ...
    const annualRate = investment?.expectedReturn.average || 6.5;
    const totalReturn = (amount * annualRate * selectedTenure) / 100;
    const maturityAmount = amount + totalReturn;

    return { totalReturn, maturityAmount };
  };

  const { totalReturn, maturityAmount } = calculateReturns();

  const handleInvest = async () => {
    if (!depositAmount) {
      Alert.alert("ত্রুটি", "জমার পরিমাণ লিখুন");
      return;
    }
    try {
      const newInvestment = {
        ...
        investAmount: parseInt(depositAmount).toFixed(0),
        total: maturityAmount,
        expectedReturn: totalReturn,
        interestRate: investment?.expectedReturn.average,
      };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.structureItem}>
          <Text style={styles.structureLabel}>সুদের হার:</Text>
          <Text style={styles.structureValue}>
            {investment?.expectedReturn.average}% (বার্ষিক)
          </Text>
        </View>
        <View style={styles.structureItem}>
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>ফিক্সড ডিপোজিট ক্যালকুলেটর</Text>
        ...
        {depositAmount && (
          <View style={styles.resultsCard}>
            ...
            <View style={styles.resultItem}>
              <Text style={styles.resultLabel}>মূল অর্থ:</Text>
              <Text style={styles.resultValue}>
                {formatCurrency(Number.parseFloat(depositAmount))}
              </Text>
            </View>
            <View style={styles.resultItem}>
              <Text style={styles.resultLabel}>
                সুদ ({selectedTenure} বছরে):
              </Text>
              <Text style={styles.resultValue}>
            ...
            <View style={styles.rateInfo}>
              <Text style={styles.rateText}>
                বার্ষিক সুদের হার: {investment?.expectedReturn.average}%
              </Text>
            </View>
          </View>
        )}
      ...
      <TouchableOpacity style={styles.investButton} onPress={handleInvest}>
        <Text style={styles.investButtonText}>ফিক্সড ডিপোজিট করুন</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
