...
export default function MutualFundScreen() {
  ...
  const investmentId = (route.params as { investmentId?: string } | undefined)
    ?.investmentId;
  const { addInvestment } = useUserStore();
  ...
  const calculateReturns = () => {
    if (investmentType === "lumpsum") {
      ...
      const maturityAmount =
        amount * Math.pow(1 + annualRate / 100, selectedTenure);
      const totalReturn = maturityAmount - amount;

      return { totalReturn, maturityAmount, totalInvestment: amount };
    } else {
      const amount = Number.parseFloat(monthlyAmount);
      if (!amount)
        return { totalInvestment: 0, maturityAmount: 0, totalReturn: 0 };
      ...
      const newInvestment = {
        type:
          investmentType === "lumpsum"
            ...
            : "mutual_fund_monthly",
        investAmount: returns.totalInvestment?.toFixed(0),
        expire: selectedTenure,
        total: returns.maturityAmount.toFixed(0),
        expectedReturn: returns.totalReturn.toFixed(0),
      };
      await addInvestment(newInvestment);
      Alert.alert("সফল", "আপনার মিউচুয়াল ফান্ড বিনিয়োগ শুরু হয়েছে!");
    } catch (error) {
      Alert.alert("ত্রুটি", "আপনার মিউচুয়াল ফান্ড বিনিয়োগ শুরু হয়েছে!");
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.typeOptions}>
          <TouchableOpacity
            style={[
              styles.typeOption,
              investmentType === "lumpsum" && styles.selectedType,
            ]}
          ...
          >
            <Text
              style={[
                styles.typeText,
                investmentType === "lumpsum" && styles.selectedTypeText,
              ]}
            >
              একবারে বিনিয়োগ
            </Text>
          ...
          <TouchableOpacity
            style={[
              styles.typeOption,
              investmentType === "sip" && styles.selectedType,
            ]}
          ...
          >
            <Text
              style={[
                styles.typeText,
                investmentType === "sip" && styles.selectedTypeText,
              ]}
            >
              SIP (মাসিক কিস্তি)
            </Text>
      ...
      <View style={styles.card}>
          ...
          <View style={styles.tenureOptions}>
            {[3, 5, 10, 15].map((years) => (
              <TouchableOpacity
                ...
                style={[
                  styles.tenureOption,
                  selectedTenure === years && styles.selectedTenure,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.tenureText,
                    selectedTenure === years && styles.selectedTenureText,
                  ]}
                >
                  {years} বছর
                </Text>
        ...
        {((investmentType === "lumpsum" && investmentAmount) ||
          (investmentType === "sip" && monthlyAmount)) && (
            ...
            {investmentType === "sip" && (
              <View style={styles.resultItem}>
                ...
                <Text style={styles.resultValue}>
                  {formatCurrency(returns.totalInvestment || 0)}
                </Text>
              </View>
            )}
            <View style={styles.resultItem}>
              <Text style={styles.resultLabel}>
                {investmentType === "lumpsum" ? "মূল অর্থ:" : "মুনাফা:"}
              </Text>
              <Text style={styles.resultValue}>
                {formatCurrency(
                  investmentType === "lumpsum"
                    ? Number.parseFloat(investmentAmount) || 0
                    : returns.totalReturn || 0
                )}
              </Text>
            </View>
            {investmentType === "lumpsum" && (
              <View style={styles.resultItem}>
                ...
                <Text style={styles.resultValue}>
                  {formatCurrency(returns.totalReturn || 0)}
                </Text>
              </View>
            )}
            <View style={styles.resultItem}>
              ...
              <Text style={styles.totalValue}>
                {formatCurrency(returns.maturityAmount || 0)}
              </Text>
            </View>
          </View>
        )}
