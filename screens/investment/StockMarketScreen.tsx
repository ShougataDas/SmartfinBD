...
export default function StockMarketScreen() {
  ...
  const investment =
    ...
    investmentOptions.find((inv) => inv.type === "stock");
  ...
  const handleInvest = async () => {
    ...
    try {
      const newInvestment = {
        ...
        investAmount: parseInt(investmentAmount).toFixed(0),
        expire: 1,
        total: (parseInt(investmentAmount) + averageReturn).toFixed(0),
        minimumReturn: minReturn.toFixed(0),
        averageReturn: averageReturn.toFixed(0),
        maxReturn: maxReturn.toFixed(0),
      };
      await addInvestment(newInvestment);
      Alert.alert("সফল", "আপনার শেয়ার বাজার বিনিয়োগ শুরু হয়েছে!");
    } catch (error) {
      Alert.alert("ত্রুটি", "আপনার শেয়ার বাজার বিনিয়োগ শুরু হয়েছে!");
    }
  };
          ...
          <View style={styles.riskOptions}>
            {[
              ...
              <TouchableOpacity
                ...
                style={[
                  styles.riskOption,
                  selectedRiskLevel === risk.key && styles.selectedRisk,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.riskText,
                    selectedRiskLevel === risk.key && styles.selectedRiskText,
                  ]}
                >
                  {risk.label}
                </Text>
          ...
          <View style={styles.resultsCard}>
            ...
            <View style={styles.warningBox}>
              <Text style={styles.warningText}>
                ⚠️ শেয়ার বাজারে বিনিয়োগ ঝুঁকিপূর্ণ। অতীতের পারফরমেন্স
                ভবিষ্যতের গ্যারান্টি নয়।
              </Text>
            </View>
          </View>
        )}
